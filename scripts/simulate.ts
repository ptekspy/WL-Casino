...
if (args.get("tune") === "true") {
  tune(Number(args.get("tuneRuns") ?? 120_000));
} else {
  report(Number(args.get("runs") ?? 1000), args.get("out"));
}
...
function tune(tuneRuns: number) {
  ...
  console.log(
    `Tuning against ${tuneRuns.toLocaleString()} rounds — target RTP ${(target * 100).toFixed(1)}% ` +
...
function report(sampleSize: number, outPath: string | undefined) {
